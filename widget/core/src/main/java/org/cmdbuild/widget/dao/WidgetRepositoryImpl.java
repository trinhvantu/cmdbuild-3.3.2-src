...
public class WidgetRepositoryImpl implements WidgetRepository {
    ...
    private List<WidgetData> getWidgetsForClass(String className, boolean activeOnly) {
        return dao.selectAll().from(WidgetDataFromDbImpl.class).where(ATTR_OWNER, EQ, className).accept(activeOnly ? q -> q.where(ATTR_IS_ACTIVE, EQ, true) : noop()).asList(WidgetData.class).stream().sorted(Ordering.natural().onResultOf(WidgetData::getIndex)).collect(toImmutableList());
    }
