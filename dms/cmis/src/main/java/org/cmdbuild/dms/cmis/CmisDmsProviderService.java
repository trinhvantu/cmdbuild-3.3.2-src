...
@Component
@MinionComponent(name = "DMS_ CMIS client", config = "org.cmdbuild.dms.cmis", watchForConfigs = "org.cmdbuild.dms")
public class CmisDmsProviderService implements DmsProviderService {
    ...
    @Nullable
    private Folder getFolderOrNull(Session session, String path) {
        try {
...
//        return map;
    ...
    private Repository doGetRepository() {
        CmisDmsRepositoryConfig repoConfig = CmisDmsRepositoryConfig.builder()
                .withUsername(configuration.getCmisUser())
                .withUrl(configuration.getCmisUrl())
                .withPassword(configuration.getCmisPassword()).build();
        return getCmisRepository(repoConfig);
    }
