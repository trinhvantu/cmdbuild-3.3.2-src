...
public class PostgresCommandRunner extends AbstractCommandRunner {
	...
	protected Options buildOptions() {
		...
		options.addOption("checkbinary", "check embedded binary postgres client tools");
	...
	protected void exec(CommandLine cmd) throws Exception {
		checkArgument(cmd.hasOption("checkbinary") ^ cmd.hasOption("checkserver") ^ cmd.hasOption("dump") ^ cmd.hasOption("restore"), "must choose ONE of 'checkbinary','checkserver','dump','restore'");
		if (cmd.hasOption("checkbinary")) {
			PostgresUtils.checkBinaries();
			System.out.println("binaries OK");
		} else if (cmd.hasOption("checkserver")) {
