...
public class IfcUtils {
    ...
    public static PackageMetaData buildPackageMetadata(Schema schema) {
        switch (schema) {
            case IFC2X3TC1:
                return new PackageMetaData(Ifc2x3tc1Package.eINSTANCE, schema, CmIoUtils.cmTmpDir().toPath());
            case IFC4:
                return new PackageMetaData(Ifc4Package.eINSTANCE, schema, CmIoUtils.cmTmpDir().toPath());
            default:
    ...
    public static Object toDynaBeanValue(@Nullable Object value) {
        if (value == null) {
            return null;
        } else if (value instanceof IfcEntry) {
            return toDynaBean((IfcEntry) value);
        } else if (value instanceof Iterable) {
            return stream((Iterable) value).map(v -> toDynaBeanValue(v)).collect(toList());
        } else {
            return value;
        }
    }
    ...
    public static Object fromDynaBeanValue(@Nullable Object value) {
        if (value != null && value instanceof MyDynaBean) {
            ...
            return stream((Iterable) value).map(v -> fromDynaBeanValue(v)).collect(toList());
        } else {
            return value;
        }
    }
