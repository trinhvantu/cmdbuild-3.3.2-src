...
public class Ws3RequestMethodHandler {
        ...
        try {
            if (hasQueryParamConstructor(param.getType())) {
                ...
                if (findAnnotation(param, QueryParam.class) != null) {
                    ...
                    name = checkNotBlank(findAnnotation(param, org.apache.cxf.jaxrs.ext.multipart.Multipart.class).value());
                ...
                boolean isDataSource = DataSource.class.isAssignableFrom(param.getType()) || DataHandler.class.isAssignableFrom(param.getType());
                if (header != null) {
                    ...
                    if (Iterable.class.isAssignableFrom(param.getType())
                            ...
                            && ((ParameterizedType) param.getParameterizedType()).getActualTypeArguments().length == 1
                            && Attachment.class.isAssignableFrom((Class) ((ParameterizedType) param.getParameterizedType()).getActualTypeArguments()[0])) {
    ...
    private class NamedParamProcessor extends ParamHandler {
        ...
        public Object apply(Ws3Request r) {
            ...
            if (Iterable.class.isAssignableFrom(classe) && r instanceof Ws3RestRequest) {
                value = ((Ws3RestRequest) r).getParams(name);
            } else {
    ...
    private class DataSourceParamProcessor extends ParamHandler {
        ...
        public DataSourceParamProcessor(Parameter param, String name, boolean isRequired) {
            super(param, checkNotBlank(name));
            this.isRequired = isRequired;
        }
        ...
        @Override
        public Object apply(Ws3Request r) {
            logger.trace("processing request part = {} with type = {}", name, param.getType());
            DataSource dataSource = r.getPartData(name);
            logger.trace("raw value =< {} >", dataSource);
            if (isRequired) {
                checkNotNull(dataSource, "missing required rest part = %s", name);
            }
            return convertDataSource(dataSource, param.getType());
        }
