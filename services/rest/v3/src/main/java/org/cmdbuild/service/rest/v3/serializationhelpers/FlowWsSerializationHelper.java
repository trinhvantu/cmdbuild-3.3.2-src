...
public class FlowWsSerializationHelper {
    ...
    public FlowWsSerializationHelper(
    ...
    public String serializeTaskDescriptionValue(@Nullable Object value) {
        if (value == null) {
            ...
            if (value instanceof LookupValue) {
                LookupValue lookup = ((LookupValue) value);
                ...
                return ((IdAndDescription) value).getDescription();
            } else {
    ...
    private Object serializeWidgetValue(@Nullable Object value) {
        if (value == null) {
            ...
            return list((ReferenceType[]) value).map(r -> map("_id", r.getId()));
        } else if (value instanceof LookupType[]) {
            return list((LookupType[]) value).map(r -> map("_id", r.getId()));
        } else if (value instanceof ReferenceType) {
            return ltEqZeroToNull(((ReferenceType) value).getId());
        } else if (value instanceof LookupType) {
            return ltEqZeroToNull(((LookupType) value).getId());
        } else {
            return value;
        }
    }
    ...
    private FluentMap<String, ?> serializeBasicTaskDefinition(Process process, TaskDefinition taskDefinition) {
        ...
        return serializeEssentialTaskDefinition(process, taskDefinition).with("instructions", taskDefinition.getInstructions(), "attributes", serializeTaskAttributes(attributesByName, taskDefinition)).accept(m -> {
    ...
    private FluentMap<String, ?> serializeEssentialTaskDefinition(Process process, TaskDefinition definition) {
        return map(
