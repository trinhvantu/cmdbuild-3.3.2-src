...
public class EtlGateWs {
    ...
    public Object readAll(WsQueryOptions wsQueryOptions, @QueryParam("include_etl_templates") @DefaultValue(FALSE) boolean includeEtlTemplates) {
        return response(paged(list(gateService.getAll()).map(e -> serializeGate(e, wsQueryOptions.isDetailed(), includeEtlTemplates)).withOnly(mapFilter(wsQueryOptions.getQuery().getFilter())), wsQueryOptions.getQuery()));
    }
    ...
    public Object readAllForClass(@HeaderParam(VIEW_MODE_HEADER_PARAM) String viewMode, @PathParam("classId") String classId, WsQueryOptions wsQueryOptions, @QueryParam("include_etl_templates") @DefaultValue(FALSE) boolean includeEtlTemplates) {
        List<Map<String, Object>> gates = list(gateService.getAll()).withOnly(e -> e.getShowOnClasses().contains(checkNotBlank(classId))).map(e -> serializeGate(e, wsQueryOptions.isDetailed(), includeEtlTemplates));
    ...
    @GET
    @Path("{gateId}/")
    public Object read(@PathParam("gateId") String gateId, @QueryParam("include_etl_templates") @DefaultValue(FALSE) boolean includeEtlTemplates) {
        EtlGate gate = gateService.getByCodeOrId(gateId);
    ...
    @Path("{gateId}/")
    @RolesAllowed(ADMIN_ETL_MODIFY_AUTHORITY)
    public Object update(@PathParam("gateId") String gateId, WsImportExportGateData data) {
        return response(serializeDetailedGate(gateService.update(data.toEtlGate().withId(gateService.getByCodeOrId(gateId).getId()).build())));
    }

    @DELETE
    @Path("{gateId}/")
    @RolesAllowed(ADMIN_ETL_MODIFY_AUTHORITY)
    public Object delete(@PathParam("gateId") Long templateId) {
        gateService.delete(templateId);
        return success();
    }
    ...
    private FluentMap<String, Object> serializeBasicGate(EtlGate gate) {
        return (FluentMap) map(
            ...
            if (gate.hasSingleHandler()) {
                m.put("_handler_type", serializeEnum(gate.getSingleHandlerType()));
            }
        });
    }
    ...
    public static class WsImportExportGateData {
        ...
        public WsImportExportGateData(
            ...
            this.processingMode = parseEnum(processingMode, EtlProcessingMode.class);
