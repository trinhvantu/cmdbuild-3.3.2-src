...
public class CardWs {
    ...
    public CardWs(UserClassService classService, UserCardService cardService, DaoService dao, CardWsSerializationHelperv3 helper) {
    ...
    public Object readOne(
        ...
        if (includeStats) {
            map.put("_attachment_count", dao.selectCount().from(DMS_MODEL_PARENT_CLASS).where(DOCUMENT_ATTR_CARD, EQ, cardId).getCount(),
    ...
    public Object readMany(@PathParam(CLASS_ID) String classId,
        ...
        DaoQueryOptions queryOptions = wsQueryOptions.getQuery().mapAttrNames(dao.getClasse(classId).getAliasToAttributeMap()).expandFulltextFilter(classe);
        ...
        if (isNotBlank(distinctAttribute)) {//TODO move count, distinct etc to /statistics endpoint (3.3)
            ...
            if (isNotBlank(selectFunctionValue) && equal(getSelectFunctionValueStoredFunction(selectFunctionValue).getOnlyOutputParameter().getName(), distinctAttribute)) {
            ...
            }).from(classId).where(queryOptions.getFilter()).run().stream().map(r -> helper.serializeAttributeValue(classe, distinctAttribute, r.asMap()).accept(m -> {
                if (count) {
            ...
            return response(paged(list, queryOptions.getOffset(), queryOptions.getLimit()));
        }
