...
public class ProcessWs {
    ...
    public ProcessWs(WorkflowConfiguration workflowConfiguration, WorkflowService workflowService, UserClassService classService, ClassSerializationHelper helper) {
    ...
    private FluentMap<String, Object> detailedResponse(Process process, boolean isAdminViewMode) {
        return helper.buildFullDetailExtendedResponse(isAdminViewMode ? classService.getExtendedClass(process.getName(), CQ_INCLUDE_INACTIVE_ELEMENTS, CQ_FOR_USER) : classService.getExtendedClass(process.getName(), CQ_FILTER_CEVICE, CQ_FOR_USER)).accept(processSpecificDataMapConsumer(process));//TODO avoind new user service query
