...
public class ClassFilterWs {
    ...
    public Object getDefaultForRoles(@PathParam(CLASS_ID) String classId, @PathParam(FILTER_ID) Long filterId) {
        return response(filterService.getDefaultFiltersForFilter(filterId).stream().filter(f -> equal(f.getDefaultForClass(), f.getFilter().getClassName())).map(f -> map(
    ...
    public Object updateWithPost(@PathParam(CLASS_ID) String classId, @PathParam("filterId") Long filterId, List<WsDefaultStoredFilter> roles) {
        ...
        List<CardFilterAsDefaultForClass> filters = roles.stream().map(r -> new CardFilterAsDefaultForClassImpl(filter, filter.getClassName(), r.getId())).collect(toList());
    ...
    public FluentMap<String, Object> serializeFilter(StoredFilter filter) {
        return map(
                ...
                "_description_translation", filter.isShared() ? translationService.translateFilterDescription(filter.getClassName(), filter.getName(), filter.getDescription()) : filter.getDescription(),
