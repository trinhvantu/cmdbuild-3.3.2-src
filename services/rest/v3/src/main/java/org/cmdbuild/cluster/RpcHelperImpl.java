...
public class RpcHelperImpl implements RpcHelper {
    ...
    public String invokeRpcMethod(@Nullable String sessionId, String payload) {
        ...
        try {
            ...
            Ws3ResponseHandler response = requestHandler.handleRequest(parseRpcRequest(payload));
            if (response.isJson()) {//TODO check also for `attachment` file download headers
                return response.getResponseAsString();
            } else {
                DataSource responseData = response.getResponseAsDataSource();
