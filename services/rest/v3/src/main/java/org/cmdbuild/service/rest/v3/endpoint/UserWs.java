...
public class UserWs {
    ...
    public UserWs(UserRepository repository, MultitenantService multitenantService, RoleRepository groupRepository, UserConfigService userPreferencesStore, SessionService sessionService, PasswordService passwordService) {
    ...
    public Object readMany(WsQueryOptions query) {
        ...
        PagedElements<UserData> users = repository.getMany(query.getQuery().mapAttrNames(USER_TABLE_ATTR_NAME_MAPPING));
        return response(users.map(defaultIfNull(query.isDetailed(), false) ? this::serializeDetailedUser : this::serializeUser), handlePositionOfAndGetMeta(queryOptions, users));
    }
    ...
    public Object update(@PathParam("userId") Long id, WsUserData data) {
        UserData user = data.toUserData().withId(id).accept(u -> {
            if (data.changePasswordRequired) {
                u.withPassword(null).withRecoveryToken(isNotBlank(data.password) ? passwordService.encryptPassword(data.password) : repository.get(id).getRecoveryTokenOrPassword());
            } else {
                u.withRecoveryToken(null).withPassword(isNotBlank(data.password) ? passwordService.encryptPassword(data.password) : repository.get(id).getPasswordOrRecoveryToken());
            }
        }).build();
