...
public class GeoValueWs {
    ...
    @GET
    @Path(EMPTY)
    public Object query(@QueryParam(ATTRIBUTE) Set<Long> attrs, @QueryParam(AREA) String area, @QueryParam("attach_nav_tree") @DefaultValue(FALSE) Boolean attachNavTree, @QueryParam(FILTER) String filterStr) {
        CmdbFilter filter = CmdbFilterUtils.parseFilter(filterStr);
        if (attachNavTree) {
            GisValuesAndNavTree geoValuesAndNavTree = gis.getGisValuesAndNavTree(attrs, area, filter);
            ...
            return geometriesToResponse(gis.getGisValues(attrs, area, filter));
        }
    }
