...
public class ClassStatsWs {
    ...
    @GET
    @Path("{b:stats}")
    public Object stats(@PathParam(CLASS_ID) String classId, WsQueryOptions wsQueryOptions, @QueryParam("query") String query) {
        Classe classe = classService.getUserClass(classId);
        StatsQueryResponse response = cardService.getStats(classId, wsQueryOptions.getQuery(), DaoStatsQueryOptionsUtils.statsQueryOptionsFromJson(query));
