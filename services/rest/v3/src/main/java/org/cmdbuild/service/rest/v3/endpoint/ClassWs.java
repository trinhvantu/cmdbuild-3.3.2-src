...
public class ClassWs {
    ...
    public Object readAll(@HeaderParam(VIEW_MODE_HEADER_PARAM) String viewMode, @QueryParam(DETAILED) @DefaultValue(FALSE) Boolean detailed, @QueryParam(LIMIT) Integer limit, @QueryParam(START) Integer offset, @QueryParam(FILTER) String filterStr) {
        List list = (isAdminViewMode(viewMode) ? classService.getAllUserClasses() : classService.getActiveUserClasses()).stream()
                .map(detailed ? compose(helper::buildFullDetailExtendedResponse, c -> isAdminViewMode(viewMode) ? classService.getExtendedClass(c, CQ_INCLUDE_INACTIVE_ELEMENTS, CQ_FOR_USER) : classService.getExtendedClass(c, CQ_FOR_USER, CQ_FILTER_CEVICE)) : helper::buildBasicResponse).collect(toList());
    ...
    public Object read(@HeaderParam(VIEW_MODE_HEADER_PARAM) String viewMode, @PathParam("classId") String classId) {
        ExtendedClass classe = isAdminViewMode(viewMode) ? classService.getExtendedClass(classId, CQ_INCLUDE_INACTIVE_ELEMENTS, CQ_FOR_USER) : classService.getExtendedClass(classId, CQ_FOR_USER, CQ_FILTER_CEVICE);
