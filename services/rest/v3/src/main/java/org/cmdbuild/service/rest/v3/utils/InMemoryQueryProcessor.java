...
import org.cmdbuild.data.filter.SorterProcessor;
    ...
    private List<Pair<Map<String, Object>, Map<String, Object>>> applyFilter(List<Pair<Map<String, Object>, Map<String, Object>>> standardDetailedList) {
        if (filter.isNoop()) {
            ...
            if (filter.hasFulltextFilter()) {
                String text = filter.getFulltextFilter().getQuery();
                standardDetailedList = standardDetailedList.stream().filter(r -> r.getRight().values().stream().filter(Objects::nonNull).anyMatch(v -> toStringOrEmpty(v).contains(text))).collect(toImmutableList());
            }
            return standardDetailedList;
        }
    }
