...
public class DmsModelWs {
    ...
    public Object readAllAttributes(@HeaderParam(VIEW_MODE_HEADER_PARAM) String viewMode, @PathParam(CLASS_ID) String classId, @QueryParam(LIMIT) Integer limit, @QueryParam(START) Integer offset) {
        List list = (isAdminViewMode(viewMode) ? getDmsModel(classId).getServiceAttributes() : getDmsModel(classId).getActiveServiceAttributes()).stream().map(a -> conversionService.serializeAttributeType(a, isAdminViewMode(viewMode))).collect(toList());
        return response(paged(list, offset, limit));
    }
    ...
    public Object createAttribute(@PathParam(CLASS_ID) String classId, WsAttributeData data) {
        ...
        return response(conversionService.serializeAttributeType(attribute, true));
    }
    ...
    public Object reorderAttributes(@PathParam(CLASS_ID) String classId, List<String> attrOrder) {
        ...
        return response(attrOrder.stream().map(getDmsModel(classId)::getAttribute).map(a -> conversionService.serializeAttributeType(a, true)).collect(toList()), attrOrder.size());
    }
