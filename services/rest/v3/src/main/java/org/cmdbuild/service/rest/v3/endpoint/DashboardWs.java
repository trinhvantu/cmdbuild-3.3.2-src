...
public class DashboardWs {
    ...
    @GET
    @Path("{id}/")
    public Object readOne(@PathParam("id") String idOrCode) {
        return response(serializeDetailedDashboard(dashboardService.getForUserByIdOrCode(idOrCode)));
    }
    ...
    private FluentMap<String, Object> serializeDetailedDashboard(DashboardData dashboard) {
        return serializeBasicDashboard(dashboard).with(
                "charts", translateCharts(fromJson(dashboard.getConfig(), JsonNode.class).get("charts"), dashboard.getCode()),
                "layout", fromJson(dashboard.getConfig(), Map.class).get("layout")//TODO improve this
        );
    }

    private JsonNode translateCharts(JsonNode charts, String dashboardCode) {
        stream(charts.elements()).map(ObjectNode.class::cast).forEach(chart -> {
            if (chart.hasNonNull("description")) {
                chart.put("_description_translation",
                        translationService.translateDashboardChartDescription(dashboardCode, chart.get("_id").asText(), chart.get("description").asText()));
            }
            if (chart.hasNonNull("valueAxisLabel")) {
                chart.put("_valueAxisLabel_translation",
                        translationService.translateDashboardChartValueAxisLabel(dashboardCode, chart.get("_id").asText(), chart.get("valueAxisLabel").asText()));
            }
            if (chart.hasNonNull("categoryAxisLabel")) {
                chart.put("_categoryAxisLabel_translation",
                        translationService.translateDashboardChartCategoryAxisLabel(dashboardCode, chart.get("_id").asText(), chart.get("categoryAxisLabel").asText()));
            }
            if(chart.hasNonNull("labelField")){
                chart.put("_labelField_translation",
                        translationService.translateDashboardChartLabelField(dashboardCode, chart.get("_id").asText(), chart.get("labelField").asText()));
            }
            if (chart.hasNonNull("dataSourceFilter")) {
                if (!chart.get("dataSourceFilter").asText().isBlank()) {
                    ObjectMapper mapper = new ObjectMapper();
                    ((ObjectNode) chart).set("ecqlDataSourceFilter", mapper.valueToTree(createEcqlFilterData(chart.get("dataSourceFilter").asText())));
                }
            }
            if (chart.hasNonNull("dataSourceParameters")) {
                ...
                for (JsonNode dataSourceParameter : dataSourceParameters) {
                    if (dataSourceParameter.hasNonNull("name")) {
                        ((ObjectNode) dataSourceParameter).put("_name_translation",
                                translationService.translateDashboardChartDataSourceParameter(dashboardCode, chart.get("_id").asText(), Integer.toString(i++), dataSourceParameter.get("name").asText()));
                    }
    ...
    private Map<String, Object> createEcqlFilterData(String cqlFilter) {
        ...
        String ecqlId = EcqlUtils.buildEmbeddedEcqlId(cqlFilter);
        Map<String, Object> ecqlData = map("id", ecqlId,
