...
public class SessionsWs extends SessionWsCommons {
    ...
    public SessionsWs(UiFilterConfiguration config, SessionService sessionService, MultitenantService multitenantService, CoreConfiguration configuration) {
    ...
    @POST
    @Path(EMPTY)
    public Object create(@Context HttpServletRequest request, WsSessionData sessionData, @QueryParam(EXT) @Nullable Boolean includeExtendedData, @QueryParam("scope") String scopeStr, @QueryParam("returnId") @DefaultValue(FALSE) Boolean returnId) {
        ...
        try {
            ...
            if (attachSessionCookie) {
                return Response.ok(responsePayload).header(SET_COOKIE, buildSetCookieHeader(CMDBUILD_AUTHORIZATION_COOKIE, sessionId, config.getCookieMaxAgeSeconds(), firstNotBlank(request.getContextPath(), "/"), config.enableCookieSecure(request.isSecure()), true, config.getCookieSameSiteMode())).build();
            } else {
                return responsePayload;
            }
        } catch (UserVisibleAuthenticationException ex) {
            return Response.status(UNAUTHORIZED).entity(failure().with("messages", buildMessageListForResponse(ERROR, true, exceptionToUserMessage(ex)))).build();
    ...
    public Object delete(@Context HttpServletRequest request, @PathParam(ID) String sessionId) {
        sessionId = sessionIdOrCurrent(sessionId);
        checkArgument(sessionService.exists(sessionId), "session not found for id = %s", sessionId);
        RequestAuthenticatorResponse<Void> response = sessionService.deleteSession(sessionId);
        return Response.ok(success().accept(m -> {
            if (response.hasRedirectUrl()) {
                m.put("redirect", response.getRedirectUrl());
            }
        })).header(SET_COOKIE, buildDeleteCookieHeader(CMDBUILD_AUTHORIZATION_COOKIE, firstNotBlank(request.getContextPath(), "/"), config.enableCookieSecure(request.isSecure()), true, config.getCookieSameSiteMode())).build();
    }
    ...
    public Object keepalive() {
        return response(map("timeToLiveSeconds", configuration.getSessionTimeoutOrDefault(), "recommendedKeepaliveIntervalSeconds", configuration.getSessionTimeoutOrDefault() / 3));
    }
    ...
    private FluentMap<String, ?> serializeSession(Session session, @Nullable Boolean includeExtendedData, boolean includeId) {
        ...
        return (FluentMap) map(
            ...
            if (firstNonNull(includeExtendedData, false)) {
                m.put(
                        "availableRolesExtendedData", user.getLoginUser().getRoleInfos().stream().map((g) -> map("_id", g.getId(), "code", g.getName(), "description", g.getDescription())).collect(toList()),
