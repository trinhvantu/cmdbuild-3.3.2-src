...
public class WsEmailData {
    ...
    private final Long autoReplyTemplate;
    ...
    public WsEmailData(
        ...
        this.status = ObjectUtils.firstNonNull(EmailUtils.parseEmailStatus(status), EmailStatus.ES_DRAFT);
        this.expr = expr;
        this.card = card;
    }

    public EmailImpl.EmailImplBuilder toEmail() {
        return EmailImpl.builder().withReplyTo(replyTo).withDelay(delay).withBccAddresses(bcc).withCcAddresses(cc).withContent(body).withContentType(contentType).withDelay(delay).withFromAddress(from).withKeepSynchronization(keepSynchronization).withNoSubjectPrefix(noSubjectPrefix).withPromptSynchronization(promptSynchronization).withStatus(status).withSubject(subject).withAutoReplyTemplate(autoReplyTemplate).withToAddresses(to).accept(b -> {
