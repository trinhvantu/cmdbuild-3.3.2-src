...
public class ExceptionHandlerService implements ExceptionMapper<Exception> {
    ...
    public static List buildResponseMessages(List<FaultEvent> events) {
        return events.stream().sorted(Ordering.from(LeveOrderErrorsFirst.INSTANCE).onResultOf(FaultEvent::getLevel)).map(ExceptionHandlerService::errorToMessage).flatMap(List::stream).collect(toList());
    }

    public static List<Object> errorToMessage(FaultEvent event) {
        return FaultUtils.errorToMessage(event).stream().map(e -> map("level", serializeLevel(e.getLevel()), "show_user", e.showUser(), "message", e.getMessage())).collect(toList());
    }

    private static String serializeLevel(FaultEventLevel level) {
        switch (level) {
            case ERROR:
