...
public class ReportUtils {
    ...
    private static List<JRDesignSubreport> searchSubreports(List<JRChild> elements) {
        ...
        while (i.hasNext()) {
            ...
            if (jreg instanceof JRDesignSubreport) {
                subreportsList.add((JRDesignSubreport) jreg);
        ...
        return (builder) -> {
            ...
            try {
                ...
                if (allCompiledReports.size() == 1) {
                    masterReport = getOnlyElement(allCompiledReports.entrySet());
                } else {
                    masterReport = allCompiledReports.entrySet().stream().filter((r) -> !getSubreports(r.getValue()).isEmpty()).collect(onlyElement());
                }
            } catch (Exception ex) {
            ...
            getImages(jasperDesign).forEach(imageHelper::prepareDesignImageForUpload);
            addSysParams(jasperDesign);

            List<JRDesignSubreport> subreports = getSubreports(jasperDesign);
            ...
            map(allCompiledReports).withoutKey(masterReport.getKey()).forEach((fileName, subreport) -> {
            ...
            list(sourceFiles.keySet()).without(imageHelper.getImageFileNames()).without(subreportFileNames).without(masterReport.getKey()).sorted().forEach((superflousFile) -> {
            ...
            List<byte[]> images = list(imageHelper.getImageFileNames()).map(sourceFiles::get),
                    compiledReports = listOf(byte[].class).with(toByteArray(jasperDesign)).with(list(subreportFileNames).map(sourceFiles::get));
    ...
    private static void addSysParams(JasperDesign report) {
        try {
            ...
            cmdb.setValueClassName("org.cmdbuild.workflow.core.fluentapi.ExtendedApi");
