...
public class GrantServiceImpl implements GrantService {
    ...
    public List<GrantData> getGrantsForRoleIncludeRecordsWithoutGrant(long roleId) {
        ...
        etlTemplateRepository.getAll().stream().filter((v) -> !grantsForRoleKeys.contains(key(POT_ETLTEMPLATE, v.getId()))).map((v) -> builder.get().withType(POT_ETLTEMPLATE).withObjectId(v.getId()).build()).forEach(list::add);

        etlGateRepository.getAll().stream().filter((v) -> !grantsForRoleKeys.contains(key(POT_ETLGATE, v.getId()))).map((v) -> builder.get().withType(POT_ETLGATE).withObjectId(v.getId()).build()).forEach(list::add);
    ...
    public String getGrantObjectDescription(GrantData grant) {
        try {
            switch (grant.getType()) {
                ...
                case POT_VIEW:
                    return viewService.getById(grant.getObjectId()).getDescription();
                case POT_ETLTEMPLATE:
                    return etlTemplateRepository.getOne(grant.getObjectId()).getDescription();
                case POT_ETLGATE:
                    return etlGateRepository.getById(grant.getObjectId()).getDescription();
                case POT_DASHBOARD:
                    return dashboardRepository.getDashboardById(grant.getObjectId()).getDescription();
                default:
    ...
    private Collection<Grant> getEtlTemplatePrivilegesByGroupId(long groupId) {
        return getPrivilegesByGroupId(PrivilegedObjectType.POT_ETLTEMPLATE, groupId, (p) -> etlTemplateRepository.getOne(p.getObjectId()));
    }

    private Collection<Grant> getEtlGatePrivilegesByGroupId(long groupId) {
        return getPrivilegesByGroupId(PrivilegedObjectType.POT_ETLGATE, groupId, (p) -> etlGateRepository.getById(p.getObjectId()));
    }
