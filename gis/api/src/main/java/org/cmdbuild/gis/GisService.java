...
public interface GisService {
    ...
    GisAttribute getGisAttribute(long attributeId);
    ...
    List<GisValue> getGisValues(Collection<Long> attrs, String bbox, CmdbFilter filter);

    GisValue setGisValue(GisValue value);

    void deleteGisValue(String classId, long cardId, String attrName);

    GisValuesAndNavTree getGisValuesAndNavTree(Collection<Long> attrs, String bbox, CmdbFilter filter);
    ...
    default GisAttribute getGisAttributeByClassAndNameOrId(String classId, String attributeNameOrLayerId) {
        if (isNumber(attributeNameOrLayerId)) {
            GisAttribute gisAttribute = getGisAttribute(parseLong(attributeNameOrLayerId));
            return gisAttribute;
        } else {
            return GisService.this.getGisAttributeIncludeInherited(classId, attributeNameOrLayerId);
        }
    }
