...
public class GisValueRepositoryImpl implements GisValueRepository {
    ...
    public Area getAreaForValues(Collection<Long> attrs) {
        ...
        String area = jdbcTemplate.queryForObject(format("SELECT st_extent(geometry) FROM (%s) g",
                attrs.stream().map(attributeRepository::getLayer).map(a -> format("SELECT \"Geometry\"::geometry geometry FROM %s", getTableName(a))).collect(joining(" UNION "))), String.class);
        if (isBlank(area)) {
            return null;
        } else {
            try {
    ...
    public List<GisValue> readGeoFeatures(GisAttribute gisAttribute, @Nullable String bbox) {
        ...
        if (isNotBlank(bbox)) {
            sql += format(" AND (_gis_table.\"Geometry\" && st_makeenvelope(%s::varchar,900913))", getEnvelopeParamsFromBboxArea(bbox));
        }
    ...
    private class GeoValuesAndNavTreeAux {
        ...
        public Pair<List<GisValue>, List<GisNavTreeNode>> getGeoValuesAndNavTree(String bbox) {
            ...
            Classe ownerClass = dao.getClasse(attr.getOwnerClassName());
