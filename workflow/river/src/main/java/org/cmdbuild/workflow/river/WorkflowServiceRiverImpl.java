...
import org.cmdbuild.workflow.river.dao.RiverPlanVersionInfo;
...
import static org.cmdbuild.workflow.river.utils.RiverFlowUtils.getRiverFlowStatus;
import static org.cmdbuild.workflow.river.utils.WfRiverXpdlUtils.checkPlanUpgradeIsSafe;
...
import org.cmdbuild.workflow.river.batch.WorkflowRiverBatchExecutorService;

@Component
public class WorkflowServiceRiverImpl implements WorkflowServiceDelegate, WorkflowRiverBatchExecutorService {
    ...
    public WorkflowServiceRiverImpl(DaoService dao, WorkflowConfiguration workflowConfiguration, ExtendedRiverPlanRepository planRepository, RiverFlowService flowService, RiverFlowConversionService flowRepository, FlowCardRepository cardRepository, RiverLockService lockService, TaskConversionService taskConversionService, TaskDefinitionConversionService taskDefinitionConversionService, FlowPersistenceService persistenceService, WorkflowTypeConverter typeConverter, AuthenticationService authenticationService, OperationUserStore user, WidgetService widgetService, CardToFlowCardWrapperService wrapperService, UserCardService cardService, EventBusService eventService) {
    ...
    private XpdlInfo doAddXpdl(String classId, DataSource dataSource, boolean replaceCurrent) {
        ...
        checkArgument(equal(xpdlClassId, classId), "xpdl binding mismatch, xpdl bind to %s = %s", ATTR_BIND_TO_CLASS, xpdlClassId);
        if (replaceCurrent) {
    ...
    public List<TaskDefinition> getTaskDefinitions(String processId) {
        ...
        if (plan == null) {
            ...
            return plan.getSteps().stream().map(Step::getTask).filter(RiverTask::isUser).map(taskDefinitionConversionService::toTaskDefinition).sorted(Ordering.natural().onResultOf(TaskDefinition::getId)).collect(toList());
        }
    }
    ...
    private abstract class ProcessOperation {
        ...
        protected void addTaskDataFromFormInFlow(Map<String, Object> data, TaskDefinition taskDefinition, List<Widget> widgets) {
            ...
            Set<String> acceptedVars = set(formVars)
                    .with(ATTR_IDTENANT);//TODO improve this; add check for user privileges (??)
