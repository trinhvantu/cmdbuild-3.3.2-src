            ...
            switch (attributeType.getName()) {
                ...
                case REFERENCE:
                    ...
                    if (targetClass.isSuperclass() && idAndDescription.hasId()) {
                        targetClass = dao.getCard(targetClass, idAndDescription.getId()).getType();
                    }
                    return buildReferenceType(idAndDescription.getId(), targetClass.getName(), idAndDescription.getDescription(), idAndDescription.getCode());
                case REFERENCEARRAY:
    ...
    private ReferenceType buildReferenceType(@Nullable Long cardId, @Nullable String classId, @Nullable String description, @Nullable String code) {
        if (isNotNullAndGtZero(cardId)) {
            try {
                ...
                Classe classe;
                if (isBlank(classId) || equal(classId, BASE_CLASS_NAME) || (description == null && code == null)) {
