...
public class FlowCardRepositoryImpl implements FlowCardRepository {
    ...
    public FlowCardRepositoryImpl(DaoService dao, LookupHelper lookupHelper, TaskEasytemplateProcessorFactory templateResolverFactory, CardToFlowCardWrapperService wrapperService, ProcessRepository planClasseRepository, UserCardService cardService, OperationUserStore user) {
    ...
    public Iterable<? extends Flow> queryOpenAndSuspended(Process processClass) {
        ...
        Optional<LookupValue> open = lookupHelper.lookupForState(OPEN);
        Optional<LookupValue> suspended = lookupHelper.lookupForState(SUSPENDED);
        Object[] ids = new Long[]{open.isPresent() ? open.get().getId() : null, suspended.isPresent() ? suspended.get().getId() : null};
        logger.debug("lookup ids are '{}'", ids);
        return dao.selectAll().from(processClass).where(ATTR_FLOW_STATUS, IN, ids).getCards().stream().map(this::cardToFlowCard).collect(toList());
    }
    ...
    private Card flowToCard(Flow flow) {
        ...
        LookupValue flowStatusLookup = lookupHelper.lookupForState(flowStatus).get();
        return CardImpl.copyOf(flow)
                .withAttribute(ATTR_FLOW_STATUS, flowStatusLookup.getId())
                .build();
    }
        ...
        private void copyValuesAndTasksFromFlowData() {
            if (flowData != null) {
                ...
                if (flowData.getStatus() != null) {
                    ...
                    Optional<LookupValue> lookup = lookupHelper.lookupForState(flowData.getStatus());
                    Object id;
                    if (lookup.isPresent()) {
                        id = lookup.get().getId();
                    } else {
