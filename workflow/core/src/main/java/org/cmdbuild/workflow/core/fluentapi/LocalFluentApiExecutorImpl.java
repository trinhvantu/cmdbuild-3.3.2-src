...
import org.cmdbuild.api.fluent.ws.AttachmentImpl;
...
public class LocalFluentApiExecutorImpl implements FluentApiExecutor {
    ...
    public LocalFluentApiExecutorImpl(
            LookupService lookupService,
            StoredFunctionService functionService,
    ...
    private org.cmdbuild.dao.beans.Card toModelCard(ExistingCard input) {
        ...
        return CardImpl.builder()
                .withType(dao.getClasse(className))
    ...
    public Card fetch(ExistingCard card) {
        return toApiCard(dao.getCard(card.getClassName(), card.getId()));
    }
    ...
    public List<Card> fetchCards(QueryClass card) {//TODO verify this method code, adapted from DataAccessLogicHelper and PrivateImpl and WsFluentApiExecutor
        Classe targetClass = dao.getClasse(card.getClassName());
...
//		return relations;
    ...
    public void upload(CardDescriptor source, Iterable<? extends Attachment> attachments) {
        attachments.forEach(a -> {
            ...
            dmsService.create(source.getClassName(), source.getId(), DocumentDataImpl.builder().withFilename(a.getName()).withCategory(a.getCategory()).withDescription(a.getDescription()).withData(data).build());
        });
    }
    ...
    public Iterable<Attachment> download(CardDescriptor source, Iterable<? extends AttachmentDescriptor> attachments) {
        return stream(attachments).map(a -> {
            ...
            return new AttachmentImpl(a.getName(), a.getDescription(), category, memoize(() -> toFileUrl(data)), () -> data);
        }).collect(toList());
    }
