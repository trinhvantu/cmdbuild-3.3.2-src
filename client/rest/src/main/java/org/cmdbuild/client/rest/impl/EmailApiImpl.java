...
public class EmailApiImpl extends AbstractServiceClientImpl implements EmailApi {
    ...
    public Email testEmailTemplate(String className, long cardId, String templateId) {
        Email email = fromJson(post(format("classes/%s/cards/%s/emails?template_only=true", UrlEscapers.urlPathSegmentEscaper().escape(checkNotBlank(className)), checkNotNullAndGtZero(cardId)), map("template", checkNotBlank(templateId))).asJackson().get("data"), WsEmailData.class).toEmail().build();
        return email;
    }
}
