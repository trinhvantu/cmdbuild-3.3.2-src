...
public class RelationApiImpl extends AbstractServiceClientImpl implements RelationApi {
    ...
    @Override
    public RelationInfo createRelation(String domain, CardIdAndClassName source, CardIdAndClassName destination) {
        return toRelationInfo(post(format("domains/%s/relations", encodeUrlPath(checkNotBlank(domain))), map(
                "_type", domain,
