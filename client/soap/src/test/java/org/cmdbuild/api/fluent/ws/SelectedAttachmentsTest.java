...
public class SelectedAttachmentsTest extends AbstractWsFluentApiTest {
	...
	private static org.cmdbuild.services.soap.client.beans.Attachment FOO;
	private static org.cmdbuild.services.soap.client.beans.Attachment BAR;
	private static org.cmdbuild.services.soap.client.beans.Attachment BAZ;
	...
	public void allAttachmentsSelected() throws Exception {
		...
		verify(proxy()).getAttachmentList((source.getClassName()),(toIntExact(source.getId())));
	...
	public void attachmentsDeleted() throws Exception {
		...
		verify(proxy(), times(2)).deleteAttachment(source.getClassName(), toIntExact(source.getId()),
	...
	public void attachmentsCopied() throws Exception {
		...
		final ArgumentCaptor<String> fileNameCaptor = ArgumentCaptor.forClass(String.class);
		verify(proxy()).getAttachmentList(source.getClassName(), toIntExact(source.getId()));
		verify(proxy(), times(2)).copyAttachment(eq(source.getClassName()), eq(toIntExact(source.getId())),
				fileNameCaptor.capture(), destination.getClassName(), toIntExact(destination.getId()));
		verifyNoMoreInteractions(proxy());
	...
	public void attachmentsMoved() throws Exception {
		...
		final ArgumentCaptor<String> fileNameCaptor = ArgumentCaptor.forClass(String.class);
		verify(proxy()).getAttachmentList(source.getClassName(), toIntExact(source.getId()));
		verify(proxy(), times(2)).moveAttachment(source.getClassName(), toIntExact(source.getId()),
				fileNameCaptor.capture(), destination.getClassName(), toIntExact(destination.getId()));
		verifyNoMoreInteractions(proxy());
	...
	private static org.cmdbuild.services.soap.client.beans.Attachment soapAttachment(final String _filename,
			final String _description, final String _category) {
		final org.cmdbuild.services.soap.client.beans.Attachment foo = new org.cmdbuild.services.soap.client.beans.Attachment() {
			{
