...
public class SendEmailJobRunner implements JobRunner {
    ...
    private class SendEmailJobRunHelper {
        ...
        private final Map<String, Object> emailContext;
        ...
        public SendEmailJobRunHelper(JobData jobData) {
            ...
            if (attachReport) {
                reportConfig = ReportConfigImpl.fromConfig(map((Map) jobData.getConfig()).filterMapKeys("attach_report_"));
        ...
        public void run() {
            ...
            Email email = templateProcessorService.applyEmailTemplate(EmailImpl.builder().withStatus(ES_OUTGOING).build(), template, emailContext);
            if (account != null) {
