...
public class JobRunImpl implements JobRun {
    ...
    private JobRunImpl(JobRunImplBuilder builder) {
        ...
        if (completed) {
            ...
            switch (jobStatus) {
                case JRS_FAILED:
                    ...
                    break;
                case JRS_COMPLETED:
                    this.hasError = builder.errorMessageData.getData().stream().anyMatch(ErrorMessageData::isError);
                    break;
                case JRS_RUNNING:
                default:
        ...
        this.metadata = map(firstNotNull(builder.metadata, emptyMap())).immutable();
    }
