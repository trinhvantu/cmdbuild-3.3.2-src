    ...
    onAfterRender: function (view) {
        ...
        vm.bind({
            bindTo: {
                ...
                errorEmailTemplate: '{theGateTemplate.errorEmailTemplate}',
                errorEmailAccount: '{theGateTemplate.errorEmailAccount}',
                fieldsadded: '{fieldsadded}'
            }
        }, function (data) {
        ...
        var dockedItems = [{
            ...
            items: CMDBuildUI.util.administration.helper.FormHelper.getOkCloseButtons({
                handler: function (_button) {
                    ...
                    var attribute = _button.up('tabpanel').down('administration-filters-attributes-panel').getAttributesData();
                    if (attribute) {
                        jsonValue.attribute = attribute;
                    }
                    var relation = _button.up('tabpanel').down('administration-filters-relations-panel').getRelationsData();
                    if (relation && relation.length) {
                        jsonValue.relation = relation;
                    }
                    var functions = _button.up('tabpanel').down('administration-components-functionfilters-panel').getFunctionData();
                    if (functions) {
                        jsonValue.functions = functions;
                    }
                    var query = _button.up('tabpanel').down('administration-components-functionfilters-panel').getFunctionData();
                    if (query) {
                        jsonValue.query = query;
                    }
                    var value;
                    if (Ext.Object.isEmpty(jsonValue)) {
                        value = '';
                    } else {
