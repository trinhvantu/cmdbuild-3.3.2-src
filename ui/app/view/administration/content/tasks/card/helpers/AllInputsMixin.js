    ...
    privates: {
        ...
        getBasicCronInput: function (vmKeyObject, attribute) {
            ...
            displayfield.renderer = function (value, input) {
                ...
                var store = input.lookupViewModel().getStore('cronSettingsStore');
                if (store && value) {
        ...
        getErrorEmailTemplateInput: function (vmKeyObject, attribute, config) {
            var _config = {};
            _config.errorEmailTemplate = Ext.merge({}, {
            ...
            return CMDBuildUI.util.administration.helper.FieldsHelper.getCommonComboInput('errorEmailTemplate', _config);
        },
                    ...
                    listeners: {
                        ...
                        beforeedit: function (grid, context) {
                            ...
                            allAttributesFiltered.addFilter(function (item) {
                                return item.canAdminShow() && !vm.get('processAttributesMapStore').findRecord('key', item.get('name')) || item.get('name') === context.record.get('key');
                            });
                        },
                ...
                items: [{
                    ...
                    listeners: {
                        ...
                        select: function (combo, record, index) {
                            ...
                            if (newEditor && newEditor.down) {
                                var combofield = newEditor.down('referencecombofield');
                                if (combofield) {
                                    combofield.setHeight(19);
                                    combofield.setMinHeight(19);
                                    combofield.setMaxHeight(19);
                                    combofield.setStyle('padding', 0);
                                    combofield.setUI('reordergrid-editor-combo');
                                }
                            }
                            valCol.setEditor(newEditor);
                        ...
                        beforeedit: function (grid, context) {
                            ...
                            this.fireEventArgs('select', [this, context.record]);
                        },
                        edit: function (tableview, context) {
                            var vm = tableview.view.lookupViewModel();
                            context.record.set('description', vm.get('allAttributesOfProcessStore').findRecord('name', context.record.get('key')).get('description'));
                    ...
                    listeners: {
                        beforerender: function (view) {
                            this.lookupViewModel().bind({
                                ...
                                view.editingPlugin.startEdit(data.store.first(), 0);
                            });
                        }
                    },
                    plugins: {
                        ...
                        placeholdersButtons: [{
                            ...
                            handler: function (button) {
                                var formGrid = button.up('grid');
                                var formGridStore = formGrid.getStore();
                                var dataGrid = button.up('administration-detailswindow').down('#processAttributesForWorkflowGrid');
                                var vm = formGrid.lookupViewModel().getParent();
                                var record = formGridStore.first();
                                var dataGridStore = vm.getStore('processAttributesMapStore');
                                var field = vm.get('precessFieldsDefinitions').find(function (item) {
                                    return item.name === record.get('key');
                                });
                                var attribute = vm.get('allAttributesOfProcessStore').findRecord('name', record.get('key'));
                                record.set('description', attribute.get('description'));

                                record.set('definition', field);
                                dataGridStore.add(record);
                                formGridStore.removeAll();
                                formGridStore.add(CMDBuildUI.model.base.KeyDescriptionValue.create());
                                formGrid.editingPlugin.startEdit(formGridStore.first(), 0);
                        ...
                        editor: {
                            ...
                            listeners: {
                                ...
                                change: function (combo, newValue, oldValue) {
                                    ...
                                    grid.getSelection()[0].set('key', newValue);
                                    var attribute = combo.lookupViewModel().get('allAttributesOfProcessStoreFiltered').findRecord('name', newValue);
                                    var valCol = grid.getColumns()[1];
                                    var newEditor = Ext.create(Ext.Object.merge({}, CMDBuildUI.util.helper.FormHelper.getEditorForField(
                                    ...
                                    newEditor.setWidth(valCol.getWidth());
                                    if (newEditor.xtype === 'referencefield') {
                                        ...
                                        if (combofield) {
                                            ...
                                            combofield.on('change', function (_input, _newValue, _oldValue) {
                                                grid.getSelection()[0].set('value', _newValue);
                                            });
                                        }
                                    } else if (newEditor.xtype === 'lookupfield') {
                                        newEditor.on('add', function (input) {
                                            input.down('lookupcombofield').on('change', function (_input, _newValue, _oldValue) {
                                                grid.getSelection()[0].set('value', _newValue);
                                            });
                                        });
                                    } else {
        ...
        getActionGateActiveInput: function (vmKeyObject, attribute, config) {
            ...
            var fieldcontainer = {
                ...
                layout: 'column',
                columnWidth: 0.5,
                fieldLabel: CMDBuildUI.locales.Locales.administration.tasks.importgis,
                localized: {
                    fieldLabel: 'CMDBuildUI.locales.Locales.administration.tasks.importgis'
                },
        ...
        getReportInput: function (vmKeyObject, attribute, disabled) {
            ...
            config[_attribute] = {
                fieldcontainer: {
                    bind: {
                        // hidden: '{theTask.config.attach_report}'
                    }
                },
                ...
                combofield: {
                    listeners: {
                        ...
                        change: function (input, newValue, oldValue) {
                            var vm = input.up('administration-content-tasks-card').lookupViewModel();
        ...
        getReportFormatInput: function (vmKeyObject, attribute, disabled) {
            ...
            config[_attribute] = {
                ...
                bind: {
                    ...
                    value: Ext.String.format('{{0}.{1}}', vmKeyObject, _attribute)
                },
                listeners: {
