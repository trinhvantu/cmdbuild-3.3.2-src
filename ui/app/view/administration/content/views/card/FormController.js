Ext.define('CMDBuildUI.view.administration.content.views.card.FormController', {
    ...
    onBeforeRender: function (view) {
        ...
        if (!vm.get('theViewFilter')) {
            if (!type) {
                var href = Ext.String.format('administration/views_empty/false/{0}', CMDBuildUI.model.views.View.types.filter);
                CMDBuildUI.util.administration.MenuStoreBuilder.selectAndRedirectToRecordBy('href', href, this);
                return;
            }
            vm.linkTo("theViewFilter", {
                type: 'CMDBuildUI.model.views.View',
                create: {
                    type: type || CMDBuildUI.model.views.View.types.filter
                }
            });
        }
    },
        ...
        var dockedItems = [{
            ...
            items: CMDBuildUI.util.administration.helper.FormHelper.getOkCloseButtons({
                handler: function (button) {
                    ...
                    var relationPanel = button.up('tabpanel').down('administration-filters-relations-panel');
                    if (relationPanel) {
                        var relations = relationPanel.getRelationsData();
            ...
            content, {}, {
                ...
                listeners: {
                    afterrender: function () {
                        var _relationPanel = this.down('administration-filters-relations-panel');
                        if (_relationPanel) {
