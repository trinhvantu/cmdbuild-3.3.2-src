Ext.define('CMDBuildUI.view.administration.content.groupsandpermissions.tabitems.permissions.components.grantconfig.tabsconfiggrid.TabsConfigModel', {
    ...
    stores: {
        gridDataStore: {
            fields: ['name', 'description', 'default', 'enable', 'disable'],
            proxy: {
                type: 'memory'
            },
            data: '{gridData}'
    ...
    createRowData: function (name, description) {
        ...
        if (Ext.isEmpty(this.get('grant').get(name))) {
            this.get('grant').set(name, '');
        }
        return {
            name: name,
            description: description,
            'default': (this.get('grant').get(name) === ''),
            'enable': (this.get('grant').get(name) === 'true'),
            'disable': (this.get('grant').get(name) === 'false')
        };
    }
