Ext.define('CMDBuildUI.view.administration.components.attributes.fieldsmanagement.group.GroupController', {
    ...
    onAddrowBtnClick: function (button) {
        ...
        this.getView().updateGroupAndRefresh();
    },
    ...
    removeColumn: function (view, rowIndex, colIndex, item, e, record, row) {
        if (record.isModel) {
            ...
            if (columns.length > 1) {
                ...
                if (!wasRemoved) {
                    CMDBuildUI.util.Notifier.showWarningMessage('Unable to remove not empty column');
                    return;
                }
                record.set('width', undefined);
    ...
    columnsSizeBtnClick: function (button) {
        ...
        var row = button.up('administration-components-attributes-fieldsmanagement-group-form-row');
        ...
        me.getView().updateGroupAndRefresh();
    },
    onAutogenerate: function () {
        ...
        view.updateGroupAndRefresh(true);
    }
});