    ...
    listeners: {
        ...
        render: function (view) {
            ...
            function allowAttribute(group, name) {
                ...
                var record = attributesStore.findRecord('name', name);
                if (record.get('group') === group.getGroup().get('_id') ||
                    (record.get('group') === '' && group.getGroup().get('_id') === CMDBuildUI.model.AttributeGrouping.nogroup)
                ) {
                    return record;
                }
                return false;
