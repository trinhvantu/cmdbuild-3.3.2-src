Ext.define('CMDBuildUI.view.bim.ContainerBimController', {
    ...
    onBeforeRender: function (view, eOpts) {
        ...
        vm.bind({
            hiddenTypes: '{bim-containerbim.hiddenTypes}'
        }, function (data) {
            if (data.hiddenTypes) {
        ...
        vm.bind({
            hiddenNode: '{bim-containerbim.hiddenNodes}'
        }, function (data) {
            if (data.hiddenNode) {
    ...
    onIfcTreeReady: function (project) {
        ...
        var ifcTreeCreation = {
            ...
            children: [this.ifcTreeCreationRecursive(ifcNode)],
            leaf: false,
            checked: true
        };
        ...
        if (selectedId) {
            ...
            if (node) {
                var globalId = node.get('oid');
                CMDBuildUI.util.bim.Viewer.select(globalId);
            }
        }
    ...
    */
    ...
    onHiglitedObject: function (highlighted) {
        ...
        var store = this.getView().down('bim-tab-cards-tree').getStore()
    ...
    onDivRendered: function (poid, selectedId) {
        ...
        CMDBuildUI.util.bim.Viewer.show(
            ...
            function () { //success callback
                ...
                if (selectedId) {
                    var node = me.getView().down('bim-tab-cards-tree').getStore().findNode('globalId', selectedId, true, true, false);
                    if (node) {
                        var globalId = node.get('oid');
                        CMDBuildUI.util.bim.Viewer.select(globalId);
                    }
