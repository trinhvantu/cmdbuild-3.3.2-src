    ...
    getErrors: function (value) {
        ...
        if (store.getCount()) {
            ...
            var haserror = store.findBy(function(record) {
                return record.get("status") === CMDBuildUI.model.dms.File.statuses.extensionNotAllowed 
                    || record.get("status") === CMDBuildUI.model.dms.File.statuses.error
                    || record.get("status") === CMDBuildUI.model.dms.File.statuses.empty;
            });
            if (haserror !== -1) {
    ...
    privates: {
        ...
        getFileStatus: function(file) {
            ...
            if (me.checkExtensions()) {
                var extension = /(?:\.([^.]+))?$/.exec(file.name)[1] || ''; // extract extension
                if (!Ext.Array.contains(Ext.Array.map(me.getAllowedExtensions(), function (ext) { return ext.toLowerCase()}), extension.toLowerCase())) {
