...
public class ForeignReferenceResolver<T extends DatabaseRecord> {

	public static abstract class EntryFiller<T extends DatabaseRecord> {
	...
	public static class ForeignReferenceResolverBuilder<T extends DatabaseRecord> implements
	...
	public static <T extends DatabaseRecord> ForeignReferenceResolverBuilder<T> newInstance() {
				...
				.transform(new Function<T, T>() {
					...
					public T apply(final T input) {
						...
						for (final String attributeName : attributes(input)) {
							final Attribute attribute = input.getType().getAttributeOrNull(attributeName);
					...
					private FluentIterable<String> inputAttributes(T input) {
						return from(input.getRawValues()) //
					...
					private FluentIterable<String> allAttributes(T input) {
						return from(input.getType().getCoreAttributes()) //
