...
public class RequestTrackingServiceImpl implements RequestTrackingService {
    ...
    public RequestTrackingServiceImpl(RecordCleanupHelperService cleanupHelper, ClusterConfiguration clusterConfiguration, RequestTrackingConfiguration config, RequestTrackingWritableRepository store, SessionService sessionService, RequestContextService contextService, SystemService bootService) {
        logger.debug("init");
    ...
    @ConfigListener(RequestTrackingConfiguration.class)
    public void cleanupRequestTable() {
        cleanupHelper.cleanupRecords(RequestData.class, config.getMaxRecordsToKeep(), config.getMaxRecordAgeToKeepSeconds());
    }
    ...
    private RequestData processRequestData(RequestData data) {
        ...
        return RequestDataImpl.copyOf(data).withUser(username).withNodeId(clusterConfiguration.getClusterNodeId()).build();
    }
