...
public class PasswordExpirationAndRecoveryServiceImpl implements PasswordRecoveryService {
    ...
    public PasswordExpirationAndRecoveryServiceImpl(PasswordManagementConfiguration pswConfig, UserRepository userRepository, DaoService dao, EmailService emailService, PasswordEncodingService encodingService) {
        ...
        if (pswConfig.isPasswordManagementEnabled() && isNotNullAndGtZero(pswConfig.getMaxPasswordAgeDays())) {
            if (isNotNullAndGtZero(pswConfig.getForewarningDays())) {
                dao.getJdbcTemplate().queryForList(format(PASSWORD_LAST_MOD_QUERY + " WHERE \"BeginDate\" BETWEEN %s AND %s",
                        systemToSqlExpr(now().minusDays(pswConfig.getMaxPasswordAgeDays()).minusDays(pswConfig.getForewarningDays())), systemToSqlExpr(now().minusDays(pswConfig.getMaxPasswordAgeDays()))), String.class).forEach((expiring) -> {
                    ...
                    if (user.isNotService() || pswConfig.isServiceUsersPasswordExpirationEnabled()) {
                        ...
                        if (user.isActive() && user.hasEmail()) {
                            emailService.create(emailService.applyTemplate(EmailImpl.builder().withStatus(ES_OUTGOING).withToAddresses(user.getEmail()).build(), EmailTemplateImpl.builder()
                                    .withSubject("[#ftl]Your password is expiring")
                                    .withContentType("text/plain")
                                    .withBody("[#ftl]\nDear ${data.username},\nyour password is expiring.\n\n")
                                    .build(), map("username", expiring)));
                        }
                    }
                });
            }
            dao.getJdbcTemplate().queryForList(format(PASSWORD_LAST_MOD_QUERY + " WHERE \"BeginDate\" < %s", systemToSqlExpr(now().minusDays(pswConfig.getMaxPasswordAgeDays()))), String.class).forEach((expired) -> {
                ...
                if (user.isNotService() || pswConfig.isServiceUsersPasswordExpirationEnabled()) {
                    user = userRepository.update(UserDataImpl.copyOf(user).withPassword("").withRecoveryToken(user.getPassword()).build());
                    if (user.isActive()) {
                        if (user.hasEmail()) {
                            emailService.create(emailService.applyTemplate(EmailImpl.builder().withStatus(ES_OUTGOING).withToAddresses(user.getEmail()).build(), EmailTemplateImpl.builder()
                                    .withName(randomId())
                                    .withSubject("[#ftl]Your password has expired")
                                    .withContentType("text/plain")
                                    .withBody("[#ftl]\nDear ${data.username},\nyour password has expired.\n\n")
                                    .build(), map("username", expired)));
                        } else {
    ...
    public void requirePasswordRecovery(String username) {
        ...
        checkArgument(userData.hasEmail(), "cannto send password recovery to user =< %s >: user does not have any email address", username);
        String recoveryToken = randomId();
        Email email = emailService.applyTemplate(EmailImpl.builder().withStatus(ES_OUTGOING).withToAddresses(userData.getEmail()).build(), EmailTemplateImpl.builder()
                .withName("_psw_recovery_template")
                .withSubject("[#ftl]Password recovery")
                .withContentType("text/plain")
                .withBody("[#ftl]\nDear ${data.username},\nhere is your recovery token = ${data.recoveryToken}\n\n")
                .build(), map("username", username, "recoveryToken", recoveryToken));
