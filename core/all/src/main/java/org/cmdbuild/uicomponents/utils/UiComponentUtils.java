...
public class UiComponentUtils {
    ...
    public static byte[] normalizeComponentData(byte[] data) {
    ...
    public static UiComponentInfo toComponentInfo(UiComponentData data) {
        return UiComponentInfoImpl.builder()
                ...
                .withId(data.getId())
                .withTargetDevice(data.getTargetDevice())//TODO check this (?)
    ...
    public static Consumer<UiComponentInfoImplBuilder> parseExtComponentData(byte[] data) {
        return b -> parseExtComponentData(data, b);
    }

    private static void parseExtComponentData(byte[] data, UiComponentInfoImplBuilder builder) {
    ...
    private static class ZipFileProcessor {
        ...
        private String cpName, alias, mainExtClass, dirPrefix, typeExpr;
        private byte[] data;
                ...
                Collections.list(zipFile.getEntries()).stream().filter(not(ZipArchiveEntry::isDirectory)).forEach((entry) -> {
                    try {
                        ...
                        if (normalizedFile.matches(".*mixins:\\[[^\\]]*['\"]((CMDBuildUI|CMDBuildMobile)\\.(mixins.CustomPage|mixins.ContextMenuComponent|view.widgets.Mixin))['\"].*")) {
                            ...
                            typeExpr = checkNotBlank(matcher.group(2));
                            mainExtClass = checkNotBlank(matcher.group(1));
                            cpName = checkNotBlank(matcher.group(4));
...
//                    .withTargetDevice(targetDevice); //TODO check this (?)
        }
