...
public class UiBaseUrlServiceImpl implements UiBaseUrlService {
    ...
    public String getBaseUrl(Object request) {
        switch (nullToEmpty(config.getBaseUrl()).toLowerCase()) {
            case AUTO_URL:
                return buildAutoUrl(request);
            case RULES_URL:
                return buildUrlWithRules(request);
            default:
                return getStaticUrl();
        }
    }
    ...
    private String buildUrlWithRules(Object request) {
        ...
        for (UiBaseUrlRule rule : config.getBaseUrlRules()) {
            if ((isNotBlank(rule.getTarget()) && Pattern.compile(rule.getTarget()).matcher(autoUrl).find())
                ...
                switch (rule.getUrl().toLowerCase()) {
                    case AUTO_URL:
                        return autoUrl;
                    default:
                        return normalizeUrl(rule.getUrl());
                }
            }
        }
        return autoUrl;
