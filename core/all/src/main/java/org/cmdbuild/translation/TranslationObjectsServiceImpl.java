...
public class TranslationObjectsServiceImpl implements TranslationObjectsService {
    ...
    public TranslationObjectsServiceImpl(
            ClasseRepository classeRepository,
            DomainRepository domainRepository,
            SharedCardFilterRepository filterRepository,
    ...
    public List<TranslationObject> getAllTranslableObjectsAndDefaults() {
        ...
        if (workflowService.isWorkflowEnabled()) {
            workflowService.getAllProcessClasses().stream().filter(p -> p.hasPlan()).forEach(p -> {
                workflowService.getTaskDefinitions(p.getName()).stream().forEach(t -> list.add(new TranslationObjectImpl(taskDescriptionTranslationCode(p.getName(), t.getId()), t.getDescription())));
            });
        }
        ...
        filterRepository.getAllSharedFilters().stream().map(f -> new TranslationObjectImpl(filterDescriptionTranslationCode(f.getClassName(), f.getName()), f.getDescription())).forEach(list::add);
        ...
        menuService.getAllMenus().stream().flatMap(m -> m.getRootNode().getDescendants().stream()).map(n -> new TranslationObjectImpl(menuitemDescriptionTranslationCode(n.getCode()), n.getDescription())).forEach(list::add);
