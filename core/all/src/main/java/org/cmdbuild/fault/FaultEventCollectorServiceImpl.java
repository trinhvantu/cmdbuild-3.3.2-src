...
public class FaultEventCollectorServiceImpl implements FaultEventCollectorService {
    ...
    public FaultEventCollectorServiceImpl(RequestContextService requestContextService, CoreConfiguration config) {
    ...
    public List buildMessagesForJsonResponse(Throwable... additionalExceptions) {
        ...
        return buildResponseMessages(collector.getCollectedEvents().stream().filter(e -> e.hasLevel(config.getNotificationMessagesLevelThreshold())).collect(toImmutableList()));
    }
    ...
    private class EventCollectorImpl implements FaultEventCollector {
        ...
        public synchronized void addEvent(FaultEvent event) {
            ...
            if (this.collectedEvents.size() >= MAX_COLLECTED_FAULT_EVENTS - 1) {
                ...
                if (this.collectedEvents.size() == MAX_COLLECTED_FAULT_EVENTS - 1) {
                    this.collectedEvents.add(new FaultEventImpl("fault event collection truncated here (too many fault events, max collector capacity exceeded)", FaultEventLevel.ERROR));
                }
            } else {
