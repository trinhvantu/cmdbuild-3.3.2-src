...
import static org.cmdbuild.view.View.ATTR_SHARED;
import static org.cmdbuild.view.View.ATTR_USER_ID;
...
public class ViewDefinitionServiceImpl implements ViewDefinitionService {
    ...
    private final JoinViewQueryService queryService;
    private final Holder<List<View>> sharedViewCache;

    public ViewDefinitionServiceImpl(DaoService dao, OperationUserSupplier userStore, CacheService cacheService, JoinViewQueryService queryService) {
    ...
    public List<View> getViewsForCurrentUser() {
        return list(getAllSharedViews()).withOnly(this::isAccessibleByCurrentUser).with(dao.selectAll().from(ViewImpl.class).where(ATTR_SHARED, EQ, false).where(ATTR_USER_ID, EQ, userStore.getUser().getId()).asList());
    }
    ...
    private boolean isAccessibleByCurrentUser(View view) {
        ...
        if (view.isShared()) {
            return user.hasPrivileges(p -> p.hasPrivileges(RP_ADMIN_VIEWS_VIEW) || p.hasReadAccess(view));
        } else {
    ...
    private void validateConfig(View view) {
        switch (view.getType()) {
            case VT_JOIN:
                queryService.validateViewConfig(view);
                break;
