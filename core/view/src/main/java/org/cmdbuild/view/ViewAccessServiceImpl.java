...
import static org.cmdbuild.view.ViewType.VT_SQL;
import static org.cmdbuild.view.ViewType.VT_FILTER;
...
public class ViewAccessServiceImpl implements ViewAccessService {
    ...
    public Card getCardById(View view, String cardId) {
        switch (view.getType()) {
            case VT_FILTER:
                return dao.getCard(view.getSourceClass(), parseLong(cardId));//TODO apply filter  
            case VT_JOIN:
                return queryService.getCard(view, cardId);
            default:
    ...
    public PagedElements<Card> getCards(View view, DaoQueryOptions queryOptions) {//TODO user permission check
        switch (view.getType()) {
            case VT_FILTER:
                return getCardsForFilterView(view, queryOptions);
            case VT_SQL:
                return getCardsForSqlView(view, queryOptions);
            case VT_JOIN:
                return queryService.getCards(view, queryOptions);
            default:
                throw unsupported("unsupported view type = %s", view.getType());
        }
    }
...
//        }
    ...
    public Collection<Attribute> getAttributesForView(View view) {
        switch (view.getType()) {
            ...
            case VT_JOIN:
                return queryService.getAttributesForView(view);
            default:
                throw unsupported("unsupported view type = %s", view.getType());
        }
    }
