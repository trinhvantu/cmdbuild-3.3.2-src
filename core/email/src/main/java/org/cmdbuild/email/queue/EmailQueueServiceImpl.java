...
@Component
@MinionComponent(name = "Email Queue", config = "org.cmdbuild.email.queue", canStartStop = true)//TODO improve namespace const
public class EmailQueueServiceImpl implements EmailQueueService {
    ...
    public MinionStatus getServiceStatus() {
        return getMinionStatus(config.isQueueEnabled());
    }

    private boolean isRunning() {
        return config.isQueueEnabled();
    }
    ...
    @ScheduledJob(value = "0 */2 * * * ?", clusterMode = RUN_ON_SINGLE_NODE, user = JOBUSER_SYSTEM)//run every 2 minutes TODO: move back to 10 minutes when jobs are made configurable
