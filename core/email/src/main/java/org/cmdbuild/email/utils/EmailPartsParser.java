...
public class EmailPartsParser {
    ...
    private void doLoadElements() {
        List<EmailAttachment> elementParts = CmCollectionUtils.list(parts).with(inline);
        ...
        htmlPart.ifPresent(inline::remove);
        plaintextPart.ifPresent(inline::remove);
        textEnrichedPart.ifPresent(inline::remove);
        attachments.addAll(inline);
    }

    private void doLoadEmailContentParts(Part part) throws MessagingException, IOException {
        logger.debug("loading email part with content type =< {} >", CmStringUtils.normalize(part.getContentType()));
        try {
            if (part.isMimeType("multipart/*")) {
                ...
                logger.trace("loaded email part with content type =< {} > disposition =< {} >", CmStringUtils.normalize(contentType), Strings.nullToEmpty(part.getDisposition()));
                EmailAttachment element = EmailAttachmentImpl.builder().withData(data).withFileName(MimeUtility.decodeText(Strings.nullToEmpty(part.getFileName()))).withContentType(contentType).withContentId(contentId).build();
                switch (Strings.nullToEmpty(part.getDisposition()).trim().toLowerCase()) {
                    case "":
                        parts.add(element);
                        break;
                    case Part.INLINE:
