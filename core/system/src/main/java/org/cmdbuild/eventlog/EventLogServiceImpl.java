...
import org.cmdbuild.config.EventLogConfiguration;
...
public class EventLogServiceImpl implements EventLogService {
    ...
    private final EventLogConfiguration config;
    ...
    public EventLogServiceImpl(FaultEventCollectorService errorAndWarningCollectorService, DaoService dao, SessionService sessionService, EventLogConfiguration config, RecordCleanupHelperService cleanupHelper, RequestContextService requestContext, EventLogEventBusService eventBusService) {
    ...
    @ConfigListener(EventLogConfiguration.class)
    public void cleanupEventLog() {
        cleanupHelper.cleanupRecords(EventLogRecord.class, config.getMaxRecordsToKeep(), config.getMaxRecordAgeToKeepSeconds());
    }
    ...
    public EventLogRecord store(String code, @Nullable Long cardId, Map<String, Object> data) {
        ...
        return store(EventLogRecordImpl.builder()
                ...
                .withErrorsData(ErrorMessagesData.fromErrorsAndWarningEvents(errorAndWarningCollectorService.getCurrentRequestEventCollectorIfExists().map(e -> e.getCollectedEvents()).orElse(emptyList())))
                .build());
    }
