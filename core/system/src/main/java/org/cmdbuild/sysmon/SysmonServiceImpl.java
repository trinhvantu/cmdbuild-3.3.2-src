...
public class SysmonServiceImpl implements SysmonService {
    ...
    public SysmonServiceImpl(SystemEventbusService eventService, CoreConfiguration coreConfiguration, SysmonRepository sysmonRepository, DirectoryService directoryService, NodeIdProvider nodeIdProvider, SessionService sessionService, BuildInfoService buildInfoService, ConfigurableDataSource dataSource) {
    ...
    private class SystemStatusChecker {
        ...
        private void checkJavaMem() {
            int javaMemMax = toIntExact((Runtime.getRuntime().maxMemory() / 1000 / 1000)),
                    javaMemTotal = toIntExact((Runtime.getRuntime().totalMemory() / 1000 / 1000)),
                    javaMemUsed = toIntExact(((Runtime.getRuntime().totalMemory() - Runtime.getRuntime().freeMemory()) / 1000 / 1000));

            int percUsed = javaMemUsed * 100 / javaMemMax;
            logger.debug("java memory = {}% used ({}/{} GB)", percUsed, javaMemUsed / 1000d, javaMemMax / 1000d);
            if (percUsed > 85) {
                addWarning("java memory almost exausted, %s%% used (%s/%s GB)", percUsed, javaMemUsed / 1000d, javaMemMax / 1000d);
        ...
        private void checkOsMemAndCpu() {
            ...
            try {
                ...
                if (file.canRead()) {
                    ...
                    long memTotal = Long.valueOf(matcher.group(1)),
                            memFree = Long.valueOf(matcher.group(2)),
                            buffers = Long.valueOf(matcher.group(3)),
                            cache = Long.valueOf(matcher.group(4));
