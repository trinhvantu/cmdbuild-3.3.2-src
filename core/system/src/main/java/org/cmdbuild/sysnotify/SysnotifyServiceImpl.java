...
public class SysnotifyServiceImpl implements SysnotifyService {
    ...
    public void notifyJobErrors(String key, List<FaultEvent> collectedEvents) {
        executorService.submit(() -> {
            try {
                FaultEventLevel notificationEventLevel = coreConfiguration.getNotificationMessagesLevelThreshold();
                sessionService.getAllSessions().stream().filter(s -> s.getOperationUser().hasPrivileges(RP_ADMIN_ACCESS)).forEach(s -> {
                    collectedEvents.stream().filter(e -> e.hasLevel(notificationEventLevel)).map(FaultUtils::errorToMessage).flatMap(List::stream).forEach(e -> {
                        String level;
                        switch (e.getLevel()) {
                            case ERROR:
