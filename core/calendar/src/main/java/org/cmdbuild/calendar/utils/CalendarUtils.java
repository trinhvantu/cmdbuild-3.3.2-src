    ...
    private static class SequenceToEventsHelper {
        ...
        private boolean shouldEndLoop(LocalDate eventDate, int generatedEventsCount) {
            if (sequence.hasFrequency(EF_ONCE) || (sequence.hasMaxActiveEvents() && (currentActiveEvents.size() + generatedEventsCount) >= sequence.getMaxActiveEvents())) {
                ...
                switch (sequence.getEndType()) {
                    case SET_DATE:
                        return !sequence.getLastEvent().isAfter(eventDate);
                    case SET_NUMBER:
                        return (currentEvents.size() + generatedEventsCount) >= sequence.getEventCount();
                    case SET_NEVER:
                        return false;
                    default:
        ...
        public List<CalendarEvent> sequenceToEvents() {
            ...
            while (true) {
                ...
                if (shouldEndLoop(eventDate, eventTimes.size())) {
                    ...
                    switch (sequence.getFrequency()) {
                        case EF_DAILY:
                            eventDate = eventDate.plusDays(sequence.getFrequencyMultiplier());
                            break;
                        case EF_WEEKLY:
                            eventDate = eventDate.plusWeeks(sequence.getFrequencyMultiplier());
                            break;
                        case EF_MONTHLY:
                            eventDate = eventDate.plusMonths(sequence.getFrequencyMultiplier());
                            break;
                        case EF_YEARLY:
                            eventDate = eventDate.plusYears(sequence.getFrequencyMultiplier());
                            break;
                        default:
