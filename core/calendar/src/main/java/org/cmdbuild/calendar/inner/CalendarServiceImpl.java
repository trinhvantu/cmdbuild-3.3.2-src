...
public class CalendarServiceImpl implements CalendarService {
    ...
    public CalendarSequence buildSequenceFromTrigger(long triggerId, LocalDate date) {
        return triggerToSequence(getTriggerById(triggerId), null, date);
    }
    ...
    public CalendarSequence updateSequence(CalendarSequence seq, Collection<CalendarEvent> events) {
        ...
        events = events.stream().map(e -> CalendarEventImpl.copyOf(e).withSequence(sequence.getId()).withSource(sequence.getSource()).withCard(sequence.getCard()).build()).map(eventRepository::createEvent).collect(toList());
        eventBus.post(CalendarEventUpdateEvent.INSTANCE);//TODO improve this
        return CalendarSequenceImpl.copyOf(sequence).withEvents(events).build();
    }
    ...
    private CalendarEvent handleTemplateExpressions(CalendarEvent event) {
        if (equal(event.getSource(), CSO_SYSTEM)) {
            ...
            return CalendarEventImpl.copyOf(event)
                    .withDescription(emailTemplateProcessorService.applyEmailTemplateExpr(card, event.getDescription()))
                    .withContent(emailTemplateProcessorService.applyEmailTemplateExpr(card, event.getContent()))
                    .build();
        } else {
            return event;
        }
