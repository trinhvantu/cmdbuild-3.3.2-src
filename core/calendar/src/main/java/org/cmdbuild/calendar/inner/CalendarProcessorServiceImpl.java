...
public class CalendarProcessorServiceImpl implements CalendarProcessorService {
    ...
    private final EmailTemplateService notificationService;
    ...
    public CalendarProcessorServiceImpl(JobSessionService sessionService, EmailService emailService, EmailTemplateService notifications, DaoService dao, CalendarService calendarService, LockService lockService, RequestContextService contextService) {
        this.calendarService = checkNotNull(calendarService);
        this.lockService = checkNotNull(lockService);
        this.dao = checkNotNull(dao);
        this.notificationService = checkNotNull(notifications);
        ...
        notificationProcessorService = executorService("calendar_notify",
                () -> contextService.initCurrentRequestContext("calendar notification processing background job"),
                contextService::destroyCurrentRequestContext);
        calendarService.getEventBus().register(new Object() {
    ...
    public void cleanup() {
        logger.info("cleanup");
        shutdownQuietly(notificationProcessorService);
    }
    ...
    private void handleExpiredNotification(long eventId, EmailTemplateInlineData notification) {
        ...
        EmailTemplate emailTemplate = notificationService.getTemplate(notification);
        if (event.getProcessedNotifications().contains(notification.getId())) {
            ...
            notificationProcessorService.submit(() -> sendNotification(event, emailTemplate));
    ...
    private void sendNotification(CalendarEvent event, EmailTemplate notification) {//TODO move this to notification service (?)
        try {
            logger.info("send event notification = {} for event = {}", notification, event);
            Email email = calendarService.buildNotificationEmail(event, notification);
            email = emailService.create(EmailImpl.copyOf(email).withStatus(ES_OUTGOING).build());//TODO handle other types of notification
