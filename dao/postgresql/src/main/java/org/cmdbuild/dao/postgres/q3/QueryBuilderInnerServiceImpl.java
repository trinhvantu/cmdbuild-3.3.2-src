...
public class QueryBuilderInnerServiceImpl implements QueryBuilderInnerService {
    ...
    public QueryBuilderInnerServiceImpl(ClasseReadonlyRepository classeRepository, DomainRepository domainRepository, PreparedQueryHelperService executorService, RefAttrHelperService refAttrHelperService, EcqlService ecqlService, QueryBuilderUtilsService utils, EntryUpdateHelperService updateHelper) {
    ...
    private class QueryBuilderProcessor {
        ...
        private QueryBuilderProcessor(QueryBuilderParams source) {
            ...
            addJoinForRefCodeDescription = source.getJoinForRefCodeDescription();
...
//                    source.filters;
            ...
            List<SelectArg> selectArgs = prepareSelectArgs(from, source.getSelectArgs(), true);
            selectArgs = selectAttrsForSorter(selectArgs);//TODO also add attrs from filters !!

            hasJoin = !source.getJoinArgs().isEmpty() || selectArgs.stream().map(SelectArg::getName).map(from::getAttributeOrNull).filter(Objects::nonNull).anyMatch(a -> a.isOfType(REFERENCE, FOREIGNKEY, LOOKUP));
        ...
        private List<SelectArg> prepareSelectArgs(EntryType entryType, List<SelectArg> selectArgs, boolean includeReservedAttrs) {
            ...
            if (isQueryMode(QM_COUNT)) {
                ...
                if (includeReservedAttrs) {
                    reservedAttrs.stream().filter(not(explicitSelectArgs::contains)).map(a -> SelectArg.build(a, quoteSqlIdentifier(a))).forEach(selectArgs::add);
                }
            ...
            if (selectAll) {
                entryType.getCoreAttributes().stream().filter(Attribute::isActive).map(Attribute::getName).filter(not(reservedAttrs::contains)).filter(not(explicitSelectArgs::contains)).sorted(Ordering.natural()).map(a -> SelectArg.build(a, quoteSqlIdentifier(a))).forEach(selectArgs::add);
            }
            return selectArgs;
        }
            ...
            switch (a.getType().getName()) {
                ...
                case FOREIGNKEY:
                    ...
                    if (a.isOfType(REFERENCE) && !domainRepository.getDomain(a.getType().as(ReferenceAttributeType.class).getDomainName()).getActiveServiceAttributes().isEmpty()) {
                        ...
                        RelationDirectionQueryHelper helper = RelationDirectionQueryHelper.forDirection(a.getType().as(ReferenceAttributeType.class).getDirection());
                        JoinQueryElement join = new JoinQueryElement(randomId(), domain, aliasBuilder.buildAlias(a.getName()));
                        join.on.add(WhereElement.build(format("%s.%s = %s.%s", join.alias, helper.getSourceCardIdExpr(), getAliasForJoinElement(JOIN_ID_DEFAULT), quoteSqlIdentifier(ATTR_ID))));
                        join.on.add(WhereElement.build(format("%s.%s = %s.%s", join.alias, helper.getTargetCardIdExpr(), getAliasForJoinElement(JOIN_ID_DEFAULT), quoteSqlIdentifier(a.getName()))));
//                        if (activeCardsOnly) { TODO improve this
                        join.on.add(WhereElement.build(format("%s.\"Status\" = 'A'", join.alias)));
//                        }
                        joinElements.add(join);
                        domain.getActiveServiceAttributes().forEach(domainAttr -> {
        ...
        private String whereArgToWhereElementExpr(WhereArg arg) {
            switch (arg.getType()) {
                case WA_OPERATOR:
                    ...
                    return buildWhereExpr(attr, attrExprExtractor, arg);
                case WA_EXPR:
                    try {
                ...
                case WA_COMPOSITE:
                    ...
                    String expr;
                    switch (arg.getCompositeOperatorNotNull()) {
                        case AND:
        ...
        private void processFilters(List<CmdbFilter> list) {
            list.forEach((filter) -> {
                getHelper().buildWheresForFilter(filter).forEach(where::add);
            });
        }
        ...
        private String buildWhereExpr(SelectElement attr, Function<SelectElement, String> attrExprExtractor, WhereArg whereArg) {
            ...
            switch (whereArg.getOperatorNotNull()) {
                case ISNULL:
                ...
                case EQ:
                    return format("%s = %s", attrExpr, onlyElementExprSupplier.get());
                case EQ_CASE_INSENSITIVE:
                    return format("LOWER(%s) = LOWER(%s)", attrExpr, onlyElementExprSupplier.get());
                case NOTEQ:
                    return format("%s <> %s", attrExpr, onlyElementExprSupplier.get());
                case LT:
                    return format("%s < %s", attrExpr, onlyElementExprSupplier.get());
                ...
                case GTEQ:
                    return format("%s >= %s", attrExpr, onlyElementExprSupplier.get());
                case IN:
                    return format("%s = ANY (%s)", attrExpr, systemToSqlExpr(set((Iterable) getOnlyElement(params)), attr.getSqlTypeHint()));
                case NOTIN:
                    return format("%s <> ALL (%s)", attrExpr, systemToSqlExpr(set((Iterable) getOnlyElement(params)), attr.getSqlTypeHint()));
                case INTERSECTS:
                    return format("%s && %s", attrExpr, systemToSqlExpr(set((Iterable) getOnlyElement(params)), attr.getSqlTypeHint()));
                case LIKE:
                    return format("%s LIKE %s", attrExpr, onlyElementExprSupplier.get());
                case MATCHES_REGEXP:
                    return format("%s ~ %s", attrExpr, onlyElementExprSupplier.get());
                ...
                case BETWEEN:
                    ...
                    return format("%s BETWEEN %s AND %s", attrExpr, systemToSqlExpr(params.get(0), attr.getSqlTypeHint()), systemToSqlExpr(params.get(1), attr.getSqlTypeHint()));
                default:
