...
BEGIN
    ...
    EXECUTE format('ALTER TABLE %s ADD COLUMN "Geometry" geometry(%s,900913) NOT NULL', _class, _type);
	RETURN _class;
...
BEGIN
	FOR _ownerclass, _attrname IN SELECT "Owner", "Code" FROM "_GisAttribute" WHERE "Id" = ANY (_gis_attributes) LOOP
		RETURN QUERY EXECUTE format('SELECT %L::regclass ownerclass,%L::varchar attrname, "Master" ownercard, st_astext("Geometry") geometry FROM gis."Gis_%s_%s" AS g WHERE g."Geometry" && st_makeenvelope(%s,900913)',
...
BEGIN
    EXECUTE format('INSERT INTO %s ("Master", "Geometry") VALUES (%L, st_geomfromtext(%L, 900913)) ON CONFLICT ("Master") DO UPDATE SET "Geometry" = EXCLUDED."Geometry"', _gis_table, _card, _value_astext);
END $$ LANGUAGE PLPGSQL;
