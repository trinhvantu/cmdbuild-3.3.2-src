...
public class PatchManagerImpl implements PatchManager {
        ...
        patchesOnDb.stream().filter(Patch::hasHash).forEach((patchOnDb) -> {
            ...
            if (patchOnFile == null) {
                ...
                if (!Objects.equal(patchOnDb.getHash(), patchOnFile.getHash())) {
                    logger.warn("hash mismatch for patch = {} \"{}\" (patch applied on db is different from patch file)", patchOnDb.getKey(), patchOnDb.getDescription());
    ...
    private void applyPatch(String sql, String patchInfo) {
        ...
        String sqlToRun = String.format("\nSET SESSION %s = 'true';\nSET SESSION %s = '{}';\n\n%s", PostgresDatabaseAdapterService.IGNORE_TENANT_POLICIES_POSTGRES_SESSION_VAR, PostgresDatabaseAdapterService.USER_TENANTS_POSTGRES_SESSION_VAR, sql);
